...
if [ -z "${BASH_VERSION:-}" ]; then
  printf '%s\n' "이 파일은 Bash 스크립트입니다. 'bash $0 ...' 형식으로 실행하세요." >&2
  exit 1
fi
...
print_usage() {
  cat <<EOF
브라우저 Mock API가 적용된 Vue + Vite 프로젝트를 GitHub Pages에 배포하는 스크립트
...
macOS 실행 예:
  bash "$SCRIPT_NAME" yangssem skala-vue ~/projects/skala-vue
...
사전 조건:
  ...
  4. 프로젝트에 src/api/staticMockAdapter.js와 .env.pages가 있어야 합니다.
EOF
}
...
ensure_node_version() {
  ...
  ' || die "이 프로젝트는 Node.js 20.19 이상 또는 22.12 이상이 필요합니다. 현재 버전: $(node --version)"
}

# macOS 기본 Bash 3.2에는 ${variable,,} 문법이 없으므로 tr을 사용합니다.
to_lower() {
...
LOWERCASE_OWNER="$(to_lower "$GITHUB_OWNER")"
LOWERCASE_REPOSITORY="$(to_lower "$REPOSITORY")"

if [[ "$LOWERCASE_REPOSITORY" == "${LOWERCASE_OWNER}.github.io" ]]; then
  PAGES_URL="https://${GITHUB_OWNER}.github.io/"
  BASE_PATH="/"
else
...
[[ -f src/api/staticMockAdapter.js ]] ||
  die "src/api/staticMockAdapter.js가 없습니다. 먼저 Pages용 브라우저 Mock API가 적용된 프로젝트를 사용하세요."

[[ -f .env.pages ]] ||
  die ".env.pages가 없습니다. Pages 빌드 모드 설정이 필요합니다."

grep -qxF "VITE_API_MODE=static" .env.pages ||
  die ".env.pages에 'VITE_API_MODE=static' 설정이 필요합니다."

grep -q "VITE_BASE_PATH" "$VITE_CONFIG" ||
  die "$VITE_CONFIG가 VITE_BASE_PATH를 base로 사용하도록 수정되어야 합니다."

node -e "const p=require('./package.json'); process.exit(p.scripts && p.scripts['build:pages'] ? 0 : 1)" ||
  die "package.json에 build:pages 스크립트가 없습니다."
...
ensure_gitignore_line "!.env.pages"

TRACKED_ENV_FILES="$(
  git ls-files '.env*' |
    while IFS= read -r file; do
      if [[ "$file" != ".env.example" && "$file" != ".env.pages" ]]; then
        printf '%s\n' "$file"
      fi
    done
)"
...
info "Pages base 경로를 '$BASE_PATH'로 사용할 예정입니다."
...
const updated = source.split('createWebHistory').join('createWebHashHistory')
...
VITE_BASE_PATH="$BASE_PATH" npm run build:pages

[[ -f dist/index.html ]] || die "빌드 결과 dist/index.html을 찾을 수 없습니다."
grep -q "${BASE_PATH}assets/" dist/index.html ||
  die "빌드 결과에 예상 base 경로 '$BASE_PATH'가 적용되지 않았습니다."
...
printf '\n'
printf '%s' "위 파일을 커밋하고 GitHub에 Push할까요? [y/N]: "
read -r CONFIRM
